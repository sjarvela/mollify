		...
		public function processGet() {
			if ($this->path[0] === 'types') {
				...
				if (count($this->path) >= 2) {
					...
					if (!$subject or !$name) throw $this->invalidRequestException();	//TODO support list requests?
					
					$types = $this->env->permissions()->getTypes();
					if (array_key_exists($name, $types["filesystem"])) {
						$item = $this->env->filesystem()->item($subject);
						$this->response()->success($this->env->permissions()->getFilesystemPermission($name, $item));
						return;
					} else {
						throw $this->invalidRequestException();	//TODO support non fs permissions as well
					}
				}
			}
